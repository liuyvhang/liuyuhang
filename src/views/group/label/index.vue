<template>
  <div class="container">
    <div class="top">
      ...
      <el-button type="primary" size="small" class="add" @click="goAdd">新增社群标签</el-button>
    </div>
    <el-table :data="tableData" border style="margin-top:20px" ref="multipleTable" tooltip-effect="dark" @selection-change="handleSelectionChange" @select="change" @select-all="change">
      ...
      <el-table-column label="社群标签名称">
        <template slot-scope="scope">
          <span @click="edit(scope.row.id)" class="title">{{scope.row.community_label_name}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="community_label_desc" label="社群标签描述">
      </el-table-column>
      <el-table-column label="社群标签图片">
        <!-- <template slot-scope="scope">
          <img :src="scope.row.community_label_pic" class="label_pic">
        </template> -->
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios.get("getLabelList", data).then(res => {
        if (res.code == 200) {
          this.tableData = res.data.communityClassifyLabelList;
    ...
    del(val) {
      ...
      this.$confirm("此操作将永久删除该社群标签, 是否继续?", "提示", {
    ...
    edit(val) {
      if (val instanceof Array) {
        ...
        this.$router.push({ path: "/group/label/edit", query: { id: val } });
      }
    },
    //发布
    putUp(val) {
    ...
    //新增标签
    goAdd() {
      this.$router.push("/group/label/add");
    }
  },
  mounted() {
    this.getData();
