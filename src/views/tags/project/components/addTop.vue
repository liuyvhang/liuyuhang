<template>
  <div>
    <el-button class="slot-t-top" type="primary" size="small" @click="dialogFormVisible = true">添加标签</el-button>
    <!-- 添加顶级节点 -->
    <el-dialog title="添加标签" :visible.sync="dialogFormVisible" @close="closeDialog('tagsData')">
    ...
    submit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          console.log(data);
          this.$axios.post("addSimilar", data).then(res => {
            console.log(res);
            console.log("添加成功");
