<template>
  <div class="container">
    <div class="top">
      ...
      <el-button type="primary" size="small" class="add" @click="goAdd">新增配置</el-button>
    </div>
    <el-table :data="tableData" border style="margin-top:20px" ref="multipleTable" tooltip-effect="dark" @selection-change="handleSelectionChange" @select="change" @select-all="change">
      ...
      <el-table-column prop="game_id" label="游戏ID" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column prop="conf_name" label="配置名称">
        <template slot-scope="scope">
          <span @click="edit(scope.row.id)" class="title">{{scope.row.conf_name}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="conf_type" label="配置类型">
        <template slot-scope="scope">
          <span>{{scope.row.conf_type==0?"积分奖励数":"单日最大奖励次数"}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="conf_val" label="配置值" :show-overflow-tooltip="true">
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    getData() {
      this.$axios
        .get("getConfig", { keyword: this.input, page: this.pn })
        .then(res => {
          console.log(res);
          if (res.code == 200) {
            this.tableData = res.data.game_conf_list;
    ...
    edit(val) {
      if (val instanceof Array) {
        this.$router.push({ path: "/game/config/edit", query: { id: val[0] } });
      } else {
        this.$router.push({ path: "/game/config/edit", query: { id: val } });
      }
    },
    //发布
    putUp(val) {
    ...
    goAdd() {
      this.$router.push("/game/config/add");
    }
  },
  mounted() {
    this.getData();
