<template>
  <div class="container">
    <div class="top">
      ...
      <el-button type="primary" size="small" class="add" @click="goAdd">新增题库</el-button>
    </div>
    <el-table :data="tableData" border style="margin-top:20px" ref="multipleTable" tooltip-effect="dark" @selection-change="handleSelectionChange" @select="change" @select-all="change">
      ...
      <el-table-column prop="question_name" label="题库名称">
        <template slot-scope="scope">
          <span @click="edit(scope.row.id)" class="title">{{scope.row.question_name}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="question_num" label="题库数量">
      </el-table-column>
      <el-table-column prop="difficulty_level" label="难度">
      </el-table-column>
      ...
      <el-table-column prop="random_type" label="随机类型">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    getData() {
      this.$axios.get("getBank", { page: this.pn }).then(res => {
        console.log(res);
        if (res.code == 200) {
    ...
    edit(val) {
      if (val instanceof Array) {
        this.$router.push({ path: "/game/bank/edit", query: { id: val[0] } });
      } else {
        this.$router.push({ path: "/game/bank/edit", query: { id: val } });
      }
    },
    //发布
    putUp(val) {
    ...
    goAdd() {
      this.$router.push("/game/bank/add");
    }
  },
  mounted() {
    this.getData();
