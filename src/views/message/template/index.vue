<template>
    <div class="container">
        <div class="top">
            ...
            <el-button type="primary" size="small" class="add" @click="goAdd">新增模板</el-button>
        </div>
        <el-table :data="tableData" border style="margin-top:20px" ref="multipleTable" tooltip-effect="dark" @selection-change="handleSelectionChange" @select="change" @select-all="change">
            ...
            <el-table-column prop="templare_name" label="模板名称">
                <template slot-scope="scope">
                    <span class="tag" @click="edit(scope.row.id)">{{scope.row.templare_name}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="templare_content" label="模板内容">
            </el-table-column>
            <el-table-column prop="is_valid_text" label="是否有效">
            </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    getData() {
      this.$axios
        .get("getTemplate", { keyword: this.input, page: this.pn })
        .then(res => {
          console.log(res);
          if (res.code == 200) {
            this.tableData = res.data.msg_tpl_list;
    ...
    goAdd() {
      this.$router.push("/message/template/add");
    }
  },
  mounted() {
    this.getData();
