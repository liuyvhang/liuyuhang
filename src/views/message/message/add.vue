<template>
    <div class="container">
        ...
        <el-form ref="formData" :model="formData" :rules="rules" label-width="80px">
            ...
            <el-form-item>
                <el-button type="primary" @click="submitForm('formData')" v-if="$route.name=='新增消息'">添加</el-button>
                <el-button type="primary" @click="editForm('formData')" v-if="$route.name=='编辑消息'">保存</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          console.log(data);
          this.$axios.post("addMessage", data).then(res => {
            if (res.code == 200) {
              this.$message({
                type: "success",
                message: "消息添加成功"
    ...
    editForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          console.log(data);
          this.$axios.post("updateMessage", data).then(res => {
            if (res.code == 200) {
              this.$message({
                type: "success",
                message: "消息编辑成功"
    ...
    getData() {
      this.$axios
        .get("infoMessage", { msg_id: this.$route.query.id })
        .then(res => {
