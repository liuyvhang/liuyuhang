  ...
  <div>
    <div class="find">
      <div class="top">
        ...
        <el-button type="primary" size="small" class="add" @click="goAdd">新增Banner</el-button>
      </div>
    </div>
    <el-table :data="tableData" border style="margin-top:20px" ref="multipleTable" tooltip-effect="dark" @selection-change="handleSelectionChange" @select="change" @select-all="change" id="#out-table">
      ...
      <el-table-column prop="title_image" label="头图" width="200">
        <template slot-scope="scope">
          <img :src="scope.row.title_image" class="img-item">
        </template>
      </el-table-column>
      ...
      <el-table-column prop="weight" label="权重" :filters="[{text: 1, value:1}, {text:2, value:2}, {text:3, value:3}, {text:4, value:4},{text:5, value:5} ]" :filter-multiple="false">
      </el-table-column>
      <el-table-column prop="status" label="状态" :filters="[{text: '已发布', value:'已发布'}, {text:'审核中', value:'审核中'}, {text:'已删除', value:'已删除'}]" :filter-multiple="false">
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    //新增功能
    goAdd() {
      this.$router.push("/news/banner/add");
    },
    //删除功能
    del(val) {
      val = val.toString();
