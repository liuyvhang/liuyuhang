  ...
  <div class="container">
    <div class="find">
      <div class="top">
        ...
        <el-button type="primary" size="small" class="add" @click="goAdd">新增视频</el-button>
      </div>
    </div>
    <el-table :data="tableData" border style="margin-top:20px" :row-style="{height:'30px'}" ref="multipleTable" tooltip-effect="dark" @selection-change="handleSelectionChange" @select="change" @select-all="change" @sort-change="sortChange" @filter-change="filterChange" id="#out-table">
      <el-table-column type="selection" width="55">
      </el-table-column>
      <el-table-column prop="id" :show-overflow-tooltip="true" label="ID">
      </el-table-column>
      <el-table-column prop="title" :show-overflow-tooltip="true" width="300" label="标题">
      ...
      <el-table-column prop="reply_num" label="回复数" sortable>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    //新增功能
    goAdd() {
      this.$router.push("/news/video/add");
    },
    //删除功能
    del(val) {
      val = val.toString();
    ...
    edit(val) {
      if (val instanceof Array) {
        this.$router.push({ path: "/news/video/edit", query: { id: val[0] } });
      } else {
        this.$router.push({ path: "/news/video/edit", query: { id: val } });
      }
    },
    //搜索
    find() {
