<template>
  <div class="container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span v-if="scope.row.conf_type==0">循环时长</span>
          <span v-if="scope.row.conf_type==1">出现频率</span>
          <span v-if="scope.row.conf_type==2">显示时长</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="conf_value"
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios.get("getDeploy", data).then(res => {
        if (res.code == 200) {
          this.tableData = res.data.ad_conf_list;
    ...
    //新增功能
    goAdd() {
      this.$router.push("/news/config/add");
    },
    //删除功能
    del(val) {
      val = val.toString();
      console.log(val);
      this.$confirm("此操作将永久删除该广告配置, 是否继续?", "提示", {
