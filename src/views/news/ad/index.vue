  ...
  <div class="container">
    <div class="find">
      <div class="top">
        ...
        <el-button type="primary" size="small" class="add" @click="goAdd">新增广告</el-button>
      </div>
    </div>
    <el-table :data="tableData" border style="margin-top:20px" ref="multipleTable" tooltip-effect="dark" @selection-change="handleSelectionChange" @select="change" @select-all="change" id="#out-table">
      ...
      <el-table-column prop="advertisement_name" label="广告名称">
        <template slot-scope="scope">
          <span @click="edit(scope.row.id)" class="title">{{scope.row.advertisement_name}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="advertisement_type" label="广告类型">
        <template slot-scope="scope">
          <span v-if="scope.row.advertisement_type==0">轮播</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="position_id" label="广告位ID" :show-overflow-tooltip="true">
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios.get("getAd", data).then(res => {
        if (res.code == 200) {
          this.tableData = res.data.advertisement_list;
    ...
    edit(val) {
      if (val instanceof Array) {
        this.$router.push({ path: "/news/ad/edit", query: { id: val[0] } });
      } else {
        this.$router.push({ path: "/news/ad/edit", query: { id: val } });
      }
    },
    //搜索
    find() {
